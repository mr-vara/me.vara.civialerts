...
global $user;
  $res = civicrm_api3('CiviAlerts', 'get', array(
 0 => array($user->name, 'Text'),
));  
  
for($j=0;$j<sizeof($res);$j++){
if($_SERVER['REQUEST_URI'] == $res[$j][3]){
if((!isset($_COOKIE['civialert']))||(isset($_COOKIE['civialert']) && (strpos($_COOKIE['civialert'], $res[$j][0]) === false)) || ($res[$j][4] == "1")){
$content = $content."
...
<div id=\"window1\" class=\"main-content-window\">
  <div class=\"content-window anim\">

	  <div class=\"my-msg\">
			<p><b>".htmlspecialchars_decode($res[$j][1])."</b> <br>
			".htmlspecialchars_decode($res[$j][2])." <br>
	<span onclick=\"document.getElementById('window1').style.display='none'\" id=\"window-closebtn\" class=\"window-close\">CLOSE</span>
		</p>
	  </div>
...
";
if($res[$j][4] == '0'){
if(!isset($_COOKIE['civialert'])){
  $cookie_name = "civialert";
  $cookie_value = $res[$j][0];
  setcookie($cookie_name, $cookie_value, time() + (86400 * 30), "/");
}
else {
if(strpos($_COOKIE['civialert'], $res[$j][0]) === false){
  $cookie_name = "civialert";
  $cookie_pre_value = $_COOKIE['civialert'];
  $cookie_value = $cookie_pre_value."-".$res[$j][0];
  setcookie($cookie_name, $cookie_value, time() + (86400 * 30), "/");	
}
 ...
 *
function civialerts_civicrm_preProcess($formName, &$form) {
 ...
 *
function civialerts_civicrm_navigationMenu(&$menu) {
  _civialerts_civix_insert_navigation_menu($menu, NULL, array(
    'label' => ts('The Page', array('domain' => 'me.vara.civialerts')),
    'name' => 'the_page',
  ...
  _civialerts_civix_navigationMenu($menu);
} // */
